...
public class IORun {
	public static void main(String []args) {
		/*
		 * *입출력 (IO) : 프로그램 상의 데이터를 외부매체로 출력하거나 
		 * 				(프로그램 -> 외부매체)
		 * 				최부매체로부터 입력받아오는 과정
		 * 				(외부매체 -> 프로그램)
		 ...
		 * 
		 * *스트림 특징
		 * 		-단방형 : 입력이면 입력만 / 출력이면 출력만 가능
		 * 		=> 입출력을 동시에 하고자 한다면,
		 * 			입력용스트림/출력용 스트림을 각각 사용
		 * 		-선입선출(FIFO): 통로를 파이프로 본다면 먼저
		 * 					보낸 데이터가 먼저 나오게 됨!
		 * 
		 * *스트림 종류
		 * 		-크기 기준으로 구분한다면 (바이트 1byte/ 문자2byte)
